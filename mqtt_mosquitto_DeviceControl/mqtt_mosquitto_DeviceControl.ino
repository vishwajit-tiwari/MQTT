...
const IPAddress mqttServerIP(192,168,30,5);
...
//  publish_dashboard(temp_celsius);
  publish_dashboard_Temp(Temperature);
  publish_dashboard_Humidity(Humidity);
  ...
  while(!MQTTclient.connected()) 
  {
    ...
    Serial.println(mqttServerIP);
      ...
      if(MQTTclient.connect(deviceID)) 
      {
        ...
        /*Publish from Arduino to the MQTT-DashBoard,
        topic: fromPub/Arduino*/
//        snprintf(topic, 32, "fromPub/%s/Temp",deviceID);
//        snprintf(msg, 64, "ToDashBoard", deviceID);
//        MQTTclient.publish(topic, msg);

        /*Publish from Arduino to the MQTT-DashBoard,
        topic: fromPub/Arduino*/
//        snprintf(topic, 32, "fromPub/%s/Humidity",deviceID);
//        snprintf(msg, 64, "ToDashBoard", deviceID);
//        MQTTclient.publish(topic, msg);
...
/***************************MQTT Publish function Temp***********************/
void publish_dashboard_Temp(float temp_celsius)
{
  char buff[64];
...
/***************************MQTT Publish function Humidity***********************/
void publish_dashboard_Humidity(float humidity)
{
  char buff[64];
